...
export async function generateTicketPDF(
  ...
  const { bg: badgeBg, fg: badgeFg } = badgeColors(
    ticket.status,
    { successBg, successFg, pendingBg, pendingFg, cancelledBg, cancelledFg }
  );
  ...
  doc
    ...
    .fillColor(secondary)
    .text(
      "This pass is strictly non-transferable. Valid photo ID may be required.",
      cardX + 40,
      cardY + cardHeight - 32,
      { width: cardWidth - 80 }
    );
  ...
  doc
    ...
    .fillColor(muted)
    .text(
      "Secure Event Ticketing Engine • Powered by EventPulse",
      0,
      pageHeight - 32,
      {
        width: pageWidth,
...
function badgeColors(
  ...
  palette: {
    ...
    cancelledBg: string; cancelledFg: string;
  }
) {
  switch (status) {
    case Status.PENDING:
