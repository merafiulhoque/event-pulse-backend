...
export async function getTicketAvailability(id) {
}
...
export async function createBooking(id, data, idempotencyKey) {
    return prisma.$transaction(async (tx) => {
        const ticketExists = await tx.ticket.findUnique({
            where: { idempotencyKey }
        });
        if (!!ticketExists)
            return [false, ticketExists];
        const now = new Date();
        const event = await tx.$queryRaw `SELECT id, capacity, "bookingStart", "bookingEnd" from "Event" where id=${id} FOR UPDATE`;
        if (!event[0]) {
